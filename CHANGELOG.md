...
## [3.1.0] · 2026-09-01 · 开源首发（Apache-2.0）
...
### Changed
- 公开版编号对齐内部最新版本（v3.1）；内部母版版本线 v2.1 → v3 → v3.1 不对外暴露
...
### Fixed
- 修复 v3.1.0 首发 commit 中 6 个文件因 PowerShell 5.1 zh-CN 区域默认 ANSI/GBK 编码导致的中文字符损坏（`README.md` / `使用说明.md` / `CHANGELOG.md` / `marketplace.json` / `.claude-plugin/plugin.json` / `docs/EXAMPLES.md`）。改用 .NET UTF-8（`[System.Text.UTF8Encoding]::new($false)`）直接写入修复。
- 修复 `scripts/tests/test_p0_fixes.py` 中残留的本地业务路径示例（脱敏为 `D:\Desktop\industry-research\reports\...`）。
- 修复 SKILL.md frontmatter 与文档内容不一致（v2.1 → v3.1；触发词补英文 `generate monthly report`；"实战示例"改"通用反推案例（脱敏）"；门禁速查编号 9.5/9 → 11 道顺序编号；已知边界编号连续化）。
...
[3.1.0]: https://github.com/ZGYXTTK/industry-report-update-skill/releases/tag/v3.1.0
[Unreleased]: https://github.com/ZGYXTTK/industry-report-update-skill/compare/v3.1.0...HEAD